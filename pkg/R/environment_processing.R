...
#' @export
	...
	.tmp_str <- stringi::stri_split_regex(str = libs, pattern = delim, simplify = TRUE, omit_empty = TRUE) |>
		unlist() |>
		as.vector();

	.tmp_str <- rlang::set_names(
		.tmp_str
		, stringi::stri_replace_all_regex(.tmp_str, pattern = "([{].*.[}])?", replacement = "", vectorize_all = TRUE) |>
			unlist()
		);

	library.cfg <- purrr::map(.tmp_str, \(x){
			ns_objs <- stringi::stri_split_regex(x, pattern = "[{}]", omit_empty = TRUE, simplify = TRUE) |>
				unlist() |>
				magrittr::extract(-1);
			...
			if (rlang::is_empty(ns_objs)){
				...
				purrr::map(
					rlang::set_names(c("[+]", "[-]"), c("include.only", "exclude"))
					, \(x){
								stringi::stri_extract_all_regex(ns_objs, pattern = paste0(x, "[a-zA-Z0-9._]+"), simplify = TRUE) |>
									stats::na.omit() |>
	...
	new.libs <- purrr::keep(names(library.cfg), \(x) !x %in% rownames(utils::installed.packages()));
	...
	invisible(
		library.cfg[
			...
			purrr::map_lgl(names(library.cfg), \(x){ magrittr::not(any(grepl(x, search()))) })
			] |>
			purrr::imap(\(x, pkg){
				tryCatch(expr = { do.call(
					what = "library"
					, args = append(
							# User syntax error can lead to both being true, so let the call to 'library()' error out:
							x[c(length(x$include) > 0, length(x[[2]]) > 0)]
							, list(package = pkg, attach.required = TRUE, quietly = !chatty, warn.conflicts = FALSE)
							))
...
#'	\itemize{
#'		\item{Added \code{\link[svDialogs]{okCancelBox}} to trigger when argument \code{safe} is \code{TRUE}}
#'		}
#'	}
#' }
#'
...
#' @export
	...
	i <- purrr::map(as.character(rlang::enexprs(...)), \(x){
			stringi::stri_split_regex(x, pattern = "[,;| ]", simplify = TRUE, omit_empty = TRUE)
		}) |>
		unlist();
	...
	if (save.dir == TRUE){
		save.dir <- tcltk::tk_choose.dir(default = getwd(), caption = "Choose the 'save' directory:")
	}
	...
	namescheck <- { c(
		...
		, multi.as.string = { any(grepl("[,;| ]", i) & !any(i %in% c("*", "all"))) }
		) |>
		purrr::keep(\(x) x) |>
		names() |>
		magrittr::extract(1);
	...
	switch(
		namescheck
		, "all" 						= {
					file.name <- ifelse(filecheck, "all", file.name);
					i <- ls(envir = env, all.names = TRUE)
				}
		, "multi.as.string" = {
					file.name <- ifelse(filecheck, "multiObs", file.name);
	...
	tmp.file <- sprintf(
		...
		, save.dir
		, if (use.prefix){
				ifelse(grepl("Global", env_ref), "", paste0(env_ref, "$"))
			} else { "" }
		, file.name
		, tstamp
		);
	...
	if (safe) {
		msg <- { sprintf(
			"Preparing to save %s[%s] to %s: \nContinue? "
			, env_ref, paste(i, collapse = ", ")
			, tmp.file
			)}

		if (svDialogs::okCancelBox(message = msg)) {
			save(list = i, envir = env, file = tmp.file, compress = "bzip2", precheck = safe)
		} else {
			cli::cli_alert_info("Canceling operation ...")
		}
	} else {
...
#' @export
	...
	from <- purrr::map(as.list(queue), \(x){
		...
		env <- if (.has_dollar){
						rlang::parse_expr(
							magrittr::extract(stringi::stri_split_fixed(rlang::as_label(obj), "$", n = 2, simplify = TRUE), 1)
							) |>
	...
	purrr::iwalk(action, \(x, i){
		.check_clean <- rlang::expr({
				if (!keep.orig){ #browser()
					obj <- queue[[i]] |> rlang::quo_get_expr() |> rlang::as_label()
					env <- queue[[i]] |> rlang::quo_get_env()
